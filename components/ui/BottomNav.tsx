...
export default function BottomNav() {
  ...
  const renderNavLink = (item: (typeof firstNavItems)[number] | (typeof lastNavItems)[number]) => {
  ...
  return (
    <>
      ...
      >
        {firstNavItems.map(renderNavLink)}
        <button
          type="button"
        ...
        >
          ...
          >
            <Plus
              className={`absolute h-8 w-8 transition duration-200 ${
                showDepositSheet ? "rotate-90 scale-0 opacity-0" : "rotate-0 scale-100 opacity-100"
              }`}
              strokeWidth={3}
            />
            <X
              className={`absolute h-8 w-8 transition duration-200 ${
                showDepositSheet ? "rotate-0 scale-100 opacity-100" : "-rotate-90 scale-0 opacity-0"
              }`}
              strokeWidth={3}
            />
          </span>
        ...
        {lastNavItems.map(renderNavLink)}
      </nav>
    </>
  )
}
